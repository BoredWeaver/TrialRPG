...
function spawnSummonsMut(state, sourceEntity, summonSpec = {}) {
  ...
  let idOrObj = summonSpec.id;
  // If effect provided as bare string in some configs
  ...
  if (Number.isFinite(Number(summonSpec.level))) {
    ...
    const base = Number(sourceEntity && sourceEntity._scaledLevel) || Number(state.player.level || 1);
    desiredLevel = Math.max(1, Math.floor(base + Number(summonSpec.levelOffset)));
  } else {
    // If id contains '-lvN' it will be parsed by buildEnemyRuntimeFromSource; otherwise default to source/party level
  ...
  for (let i = 0; i < count; i++) {
    // if idOrObj is an object that contains baseId & level, pass through; otherwise, construct string
    let spawnRef = idOrObj;

    if (!spawnRef) continue;

    // If the caller passed a base id and we computed desiredLevel, create "base-lvN"
    if (typeof spawnRef === "string" && desiredLevel) {
      // If spawnRef already has a lv pattern, respect it
      const parsed = parseScaledId(spawnRef);
      if (parsed) {
        spawnRef = spawnRef; // keep as-is
      } else {
        spawnRef = `${spawnRef}-lv${desiredLevel}`;
      }
    } else if (typeof spawnRef === "object" && desiredLevel) {
      spawnRef = { ...spawnRef, level: desiredLevel };
    }

    // Build runtime enemy from the source ref using existing builder (pass enemies DB)
    const { enemies: newOnes } = buildEnemyRuntimeFromSource(spawnRef, enemies);
    if (!Array.isArray(newOnes) || newOnes.length === 0) continue;

    // Insert each constructed runtime enemy
    for (const n of newOnes) {
      // Tag summon metadata
      n._summon = true;
      n._summonOwner = sourceEntity ? (sourceEntity.id || sourceEntity.name || "unknown") : "unknown";
      // give a distinct runtime id (avoid colliding with template id)
      n.id = n.id ? `${n.id}-summon-${Date.now()}-${Math.floor(Math.random() * 1000)}` : `summon-${Date.now()}-${Math.floor(Math.random() * 1000)}`;

      // ensure runtime fields and derived stats are set
      try {
  ...
  // Update state's primary / enemy view
  state.enemy = state.enemies.length > 0 ? { ...state.enemies[0] } : null;

  return created;
}

/**
 * processEffectForEntity(state, sourceEntity, effect)
 * - handles 'summon' effects specially (calls spawnSummonsMut)
 * - otherwise falls back to pushStatusOntoEntity for existing status effects
 *
 * Example effect for summon:
 * { type: "summon", id: "goblin", count: 2, levelOffset: 0 }
 * or
 * { type: "summon", id: "goblin-lv4", count: 1 }
 */
function processEffectForEntity(state, sourceEntity, effect) {
  if (!effect || !effect.type) return;

  if (effect.type === "summon") {
    // Accept both "id" and "baseId" forms
    const spec = {
    ...
    if (Array.isArray(created) && created.length > 0) {
      ...
      try {
        emit("toast", { message: `${sourceEntity?.name || "Enemy"} summoned ${created.length} minion(s).`, type: "info" });
      } catch (e) { /* ignore toast failures */ }
    }
    return;
  }

  // For any other effect, reuse the existing status pipeline.
  try {
    pushStatusOntoEntity(sourceEntity, effect);
  } catch (e) {
    // fallback: try pushing onto a targetless status on player (rare)
    try {
      pushStatusOntoEntity(state.player, effect);
    } catch (ee) { /* ignore */ }
  }
}
